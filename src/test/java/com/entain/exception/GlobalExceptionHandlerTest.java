...
class GlobalExceptionHandlerTest {
    ...
    void handleEventNotFound_returns404() {
        ...
        assertEquals(SPORT_EVENT + id + NOT_FOUND, response.getBody());
    }
    ...
    void handleInvalidStatus_returns400() {
        ...
        assertEquals(INVALID_STATUS_CHANGE_ATTEMPT, response.getBody());
    }
    ...
    void handleInvalidJson_generic_returns400() {
        ...
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
        assertEquals(INVALID_REQUEST_BODY, response.getBody());
    }
}